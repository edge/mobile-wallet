...
//  xe_wallet
//
//  Created by Paul Davis on 24/12/2021.
//

import Foundation
import UIKit

open class BouncyLayout2: UICollectionViewFlowLayout {
    ...
    private func addItem(_ item: UICollectionViewLayoutAttributes, in view: UICollectionView) {
        let behavior = UIAttachmentBehavior(item: item, attachedToAnchor: floor2(item.center))
    ...
    private func update(behavior: UIAttachmentBehavior, and item: UIDynamicItem, in view: UICollectionView, for bounds: CGRect) {
        ...
        item.center = floor2(item.center)
    }
...
fileprivate func floor2(_ point: CGPoint) -> CGPoint {
